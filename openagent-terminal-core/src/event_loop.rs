...
pub enum Msg {
    ...
    /// Indicates that the `EventLoop` should shut down, as OpenAgent Terminal is shutting down.
    Shutdown,
...
// Parse the incoming bytes.
    ...
    /// Filter OSC 133 (iTerm2 shell integration) sequences out of the raw PTY stream,
    /// emitting CommandBlock events and returning the remaining bytes for VTE parsing.
    fn filter_and_send_osc133(&self, state: &mut State, data: &[u8]) -> Vec<u8> {
        // Concatenate with any pending partial sequence from previous reads.
        let mut combined: Vec<u8> = Vec::with_capacity(state.osc133_accum.len() + data.len());
        if !state.osc133_accum.is_empty() {
            combined.extend_from_slice(&state.osc133_accum);
            state.osc133_accum.clear();
        }
        ...
        while i < combined.len() {
            ...
            if combined[i] == ESC && i + 1 < combined.len() && combined[i + 1] == RIGHT_BRACKET {
                // Potential OSC: try to see if it's 133;
                let start = i + 2;
                // If not enough bytes to compare prefix, buffer and break.
                if start + prefix.len() > combined.len() {
                    state.osc133_accum.extend_from_slice(&combined[i..]);
                    break;
                }
                if &combined[start..start + prefix.len()] == prefix {
                    // Find terminator (BEL or ST (ESC \\)).
                    let mut j = start + prefix.len();
                    let mut term_pos: Option<usize> = None;
    ...
    /// Handle a single OSC 133 payload (after "133;").
    fn handle_osc133_payload(&self, payload: &[u8]) {
        // Expected formats:
        //  A
        //  B;cmd=<raw>
        //  C;exit=<n>;cwd=<path>
        ...
        match payload[0] as char {
            ...
            'C' => {
                ...
                // split by ';'
                let mut parts = payload[1..].split(|&b| b == b';');
...
pub struct State {
    ...
    /// Accumulator for partial OSC 133 sequences spanning multiple reads.
    osc133_accum: Vec<u8>,
}
