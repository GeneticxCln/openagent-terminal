...
///
/// Preferred config search order (UNIX):
///
/// 1. $XDG_CONFIG_HOME/openagent-terminal/openagent-terminal.{suffix}
/// 2. $XDG_CONFIG_HOME/openagent-terminal.{suffix}
/// 3. $HOME/.config/openagent-terminal/openagent-terminal.{suffix}
/// 4. $HOME/.openagent-terminal.{suffix}
/// 5. /etc/openagent-terminal/openagent-terminal.{suffix}
///
/// With backwards-compatibility fallbacks to legacy Alacritty paths.
#[cfg(not(windows))]
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let new_file_name = format!("openagent-terminal.{suffix}");

    // Prefer new XDG locations first.
    if let Some(path) = xdg::BaseDirectories::with_prefix("openagent-terminal")
        .find_config_file(&new_file_name)
        .or_else(|| xdg::BaseDirectories::new().find_config_file(&new_file_name))
    {
        return Some(path);
    }
    ...
    if let Ok(home) = env::var("HOME") {
        ...
        let hidden = PathBuf::from(&home).join(format!(".{new_file_name}"));
        if hidden.exists() {
            return Some(hidden);
        }
    }

    let etc_path = PathBuf::from("/etc/openagent-terminal").join(&new_file_name);
    if etc_path.exists() {
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    ...
    if let Some(path) = dirs::config_dir()
        .map(|path| path.join("openagent-terminal").join(&new_file_name))
        .filter(|p| p.exists())
    {
        return Some(path);
    }
