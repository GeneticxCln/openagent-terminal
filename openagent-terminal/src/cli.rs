...
pub struct Options {
    ...
    /// X11 window ID to embed OpenAgent Terminal within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
...
pub struct WindowIdentity {
    /// Defines the window title [default: OpenAgent Terminal].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: OpenAgent Terminal].
...
pub enum SocketMessage {
    /// Create a new window in the same OpenAgent Terminal process.
    CreateWindow(WindowOptions),

    /// Update the OpenAgent Terminal configuration.
    Config(IpcConfig),
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "openagent-terminal.bash"),
            (Shell::Fish, "openagent-terminal.fish"),
            (Shell::Zsh, "_openagent-terminal"),
        ] {
        ...
        // Set REBUILD_COMPLETIONS=1 to regenerate files under extra/completions/.
        if std::env::var("REBUILD_COMPLETIONS").is_ok() {
            use std::fs::File;
            let out_dir = std::path::Path::new("../extra/completions");
            std::fs::create_dir_all(out_dir).unwrap();

            let mut file = File::create(out_dir.join("openagent-terminal.bash")).unwrap();
            clap_complete::generate(Shell::Bash, &mut clap, "openagent-terminal", &mut file);

            let mut file = File::create(out_dir.join("openagent-terminal.fish")).unwrap();
            clap_complete::generate(Shell::Fish, &mut clap, "openagent-terminal", &mut file);

            let mut file = File::create(out_dir.join("_openagent-terminal")).unwrap();
            clap_complete::generate(Shell::Zsh, &mut clap, "openagent-terminal", &mut file);
        }
    }
}
