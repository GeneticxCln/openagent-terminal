...
pub fn derive_recursive<T>(
    ...
    quote! {
        ...
        impl <'de, #constrained> #cfg_crate::SerdeReplace for #ident <#unconstrained> {
            fn replace(&mut self, value: toml::Value) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for field in fields {
        ...
        if flatten && flattened_arm.is_some() {
            ...
            flattened_arm = Some(quote! {
                _ => #cfg_crate::SerdeReplace::replace(&mut self.#ident, value)?,
            });
        } else {
            ...
            stream.extend(quote! {
                    #(#aliases)|* | #literal => #cfg_crate::SerdeReplace::replace(&mut self.#ident, next_value)?,
            });
        }
    }
