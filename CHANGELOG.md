...
All notable changes to OpenAgent Terminal are documented in this file.
...
- Vi motions `*`, `#`, `{`, and `}`
- IPC config retrieval using `openagent-terminal msg get-config`
...
- Config option `window.level = "AlwaysOnTop"` to force OpenAgent Terminal to always be the toplevel window
- Escape sequence to move cursor forward tabs ( CSI Ps I )
- Pass activation token in `openagent-terminal msg create-window` on Wayland/X11
...
- `openagent-terminal migrate` crashing with recursive toml imports
...
- `openagent-terminal migrate` support for TOML configuration changes
- Headless mode using `openagent-terminal --daemon`
...
- OpenAgent Terminal not being properly activated with startup notify
...
- Autokey no longer working with OpenAgent Terminal on X11
...
- `openagent-terminal migrate` will ignore null values in yaml instead of erroring out

### Fixed

- `openagent-terminal migrate` failing with nonexistent imports
...
- `--option` argument for `openagent-terminal msg create-window`
...
Run `openagent-terminal migrate` to automatically convert all configuration files
...
- Blocking paste freezing OpenAgent Terminal on Wayland
...
- On macOS, OpenAgent Terminal now reads `AppleFontSmoothing` from user defaults to control font smoothing
...
- Windows `Open OpenAgent Terminal Here` on root of drive displaying error
- On macOS, `font.use_thin_strokes` did not work since Big Sur
- On macOS, trying to load a disabled font would crash
- On macOS, OpenAgent Terminal sessions did not appear in the list of tty sessions for `w` and `who`
...
- New `extra/openagent-terminal-msg.man` manpage for the `openagent-terminal msg` subcommand
...
- Support for running multiple windows from a single OpenAgent Terminal instance (see docs/features.md)
...
- Newly installed fonts not rendering until OpenAgent Terminal restart
...
- OpenAgent Terminal failing to start on X11 with invalid DPI reported by XRandr
...
- Escapes `CSI Ps b` and `CSI Ps Z` with large parameters locking up OpenAgent Terminal
...
- No live config update when starting OpenAgent Terminal with a broken configuration file
- PTY not drained to the end with the `--hold` flag enabled
- High CPU usage on BSD with live config reload enabled
- OpenAgent Terminal not discarding invalid escape sequences starting with ESC
...
- OpenAgent Terminal crashing at startup when the configured font does not exist
...
- On macOS, OpenAgent Terminal now requests NSSystemAdministrationUsageDescription to
...
- On Windows, OpenAgent Terminal will now use the native DirectWrite font API
...
- OpenAgent Terminal now has a fixed minimum supported Rust version of 1.31.0
...
- On Linux, the .desktop file now uses `OpenAgent Terminal` as icon name, which can be
    found at `extra/logo/openagent-terminal-term.svg`
...
- Text will reflow instead of truncating when resizing OpenAgent Terminal
